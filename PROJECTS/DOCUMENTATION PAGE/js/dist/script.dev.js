...
window.addEventListener('load', function () {
  ...
  var lightModeOn = localStorage.getItem('light-mode-on');
  ...
  document.querySelector('.light-mode-btn').addEventListener('click', function () {
    turnLightModeOn();
    if (lightModeOn == 0) lightModeOn = 1;else lightModeOn = 0;
  ...
  document.querySelector('.search-bar').addEventListener('keyup', function () {
    var input, filter, documentTitles, listCoursesLink, listCoursesItem, textValue;
    var isFound = false;
    ...
    for (var _i2 = 0; _i2 < listCoursesLink.length; _i2++) {
      textValue = removeVietnameseTones(listCoursesLink[_i2].innerText) || removeVietnameseTones(listCoursesLink[_i2].textContent);

      if (textValue.toUpperCase().indexOf(filter) > -1) {
        listCoursesItem[_i2].style.display = '';
        isFound = true;
      } else {
        listCoursesItem[_i2].style.display = 'none';
      }
    }
    ...
    if (!isFound) {
      ...
      for (var _i3 = 0; _i3 < listCoursesLink.length; _i3++) {
        listCoursesItem[_i3].style.display = '';
      }
    }
  });
});