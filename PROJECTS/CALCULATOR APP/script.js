...
const isValid = (calculation) => {
    valid = true;
    if (calculation != "") {
        let lastChar = calculation[calculation.length - 1];
        if (
            calculation[0] === "x" ||
            calculation[0] === ":" ||
            calculation[0] === "%" ||
        ...
        if (calculation.length >= 2)
            for (let i = 0; i < calculation.length - 1; i++)
                if (
                    (calculation[i] < "0" || calculation[i] > "9") &&
                    (calculation[i + 1] < "0" || calculation[i + 1] > "9")
                )
                    valid = false;
    }
...
const backSpace = () => {
    if (equalPressed) {
        ...
        if (!lastCharIsANumber(calculation)) {
            if (tempTray.length > 1) tempTray.splice(tempTray.length - 1, 1);
            else return;
            startIndex = 0;
        }
...
const add = () => {
    if (!isValid(calculation)) {
        calculationScreen.innerText = "INVALID SYNTAX!";
        return;
    } else {
        equalPressed = false;
        if (calculation === "") {
        ...
        tempTray.push(
            Number.parseFloat(calculation.slice(startIndex, calculation.length))
        );
...
const equal = () => {
    if (!isValid(calculation) || calculation === "") {
        ...
        if (tempTray.length > 1) {
            ...
            calculation =
                "0+" + calculation.slice(startIndex, calculation.length);
